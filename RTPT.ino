...
#define PAN_MIN_PULSE 1005
#define PAN_MAX_PULSE 1239
...
void calculate(double *array, double futureOffsetHours, double LST, int seconds) {
 ...
 // Get Unix time for adjusted date/time (with seconds)
 ...
 // TT correction (approx) in days: 69 seconds
 double TT = d + (69.0 / 86400.0);
 double T = TT / 36525.0;

 // Check which object is selected
 if (planet == 10) {
 // Calculate Moon position (simplified Meeus approach)
 ...
 // Convert distance Earth radii to AU (1 Earth radius = 0.000042635 AU)
 ...
 // Convert from ecliptic spherical coords to rectangular coordinates (geocentric)
 double x = dist_au * cos(lat_rad) * cos(lon_rad);
 double y = dist_au * cos(lat_rad) * sin(lon_rad);
 double z = dist_au * sin(lat_rad);

 // Convert ecliptic coords to equatorial coords
 double ec = radians(23.439292);  // obliquity of ecliptic
 double Xq = x;
 ...
 array[0] = degrees(alpha) / 15.0;  // RA in hours
 array[1] = degrees(delta);         // Dec in degrees
 array[2] = distance;               // Distance in AU

 // Now calculate horizontal coordinates and servo angles same as planets
 // Use passed LST (already radians)
 ...
 az = degrees(az);
 az = fmod(az + 180.0, 360.0);  // correct for coordinate system flip
 ...
 bool isDwarf = (planet >= 11 && planet <= 14);

 // Meeus planet arrays
 double a_planet[] = {0,0.387098,0.723330,1.000000,1.523688,5.20260,9.55491,19.2184,30.1104,39.4821};
 double e_planet[] = {0,
   0.205630 + 0.000020406 * T,
   0.006773 - 0.000049 * T,
   0.016710 - 0.000042037 * T,
   0.093312 + 0.000092064 * T,
   0.048334 + 0.00016418 * T,
   0.055892 - 0.000346818 * T,
   0.046344 - 0.00002658 * T,
   0.008997 + 0.00000633 * T,
   0.24880766
 };
 double i_planet[] = {0,
   7.00487 - 23.51e-6 * T,
 ...
 double Omega_planet[] = {0,
   48.33167 - 446.30e-6 * T,
   76.68069 - 996.89e-6 * T,
   0.0,
   49.57854 - 1.105e-5 * T,
   100.55615 + 1.019e-5 * T,
   113.71504 - 1.143e-5 * T,
   74.22988 - 1.578e-5 * T,
   131.72169 - 1.151e-5 * T,
   110.30347
 };
 double Pi_planet[] = {0,
   77.45645 + 573.57e-6 * T,
   131.53298 - 108.80e-6 * T,
   102.94719 + 1198.28e-6 * T,
   336.04084 + 1590.20e-6 * T,
   14.75385 + 870.53e-6 * T,
   92.43194 + 541.14e-6 * T,
   170.96424 + 408.52e-6 * T,
   44.97135 + 292.73e-6 * T,
   224.06676
 };
 double L_planet[] = {0,
 ...
 // Dwarf planet arrays
 double a_dwarf[] = {2.76605,42.96094,45.44941,68.050601};
 double e_dwarf[] = {0.079401,0.197035,0.161942,0.435675};
 double i_dwarf[] = {10.5877,28.2083,29.033859,43.82162};
 double Omega_dwarf[] = {80.2522,121.82535,79.25967,36.046036};
 double omega_dwarf[] = {73.27345,-119.02506,-63.04815,150.143634};
 double n_dwarf[] = {0.214,0.00396,0.00364,0.00194};
 double Tp_dwarf[] = {2460800.5,2460800.5,2460800.5,2460800.5};

 double M, ecc, r, i_rad, o_rad, w_rad;

if (isDwarf) {
 int k = planet - 11;
 double JD = d + 2451545.0;
 M = radians(fmod(n_dwarf[k] * (JD - Tp_dwarf[k]), 360.0));
 ecc = e_dwarf[k];
 double v = M + (2 * ecc - 0.25 * pow(ecc, 3)) * sin(M)
              + (1.25 * pow(ecc, 2)) * sin(2 * M)
              + ((13.0 / 12.0) * pow(ecc, 3)) * sin(3 * M);
 r = a_dwarf[k] * (1 - ecc * ecc) / (1 + ecc * cos(v));
 i_rad = radians(i_dwarf[k]);
 o_rad = radians(Omega_dwarf[k]);
 w_rad = radians(omega_dwarf[k]);
 ...
 double ME = radians(fmod(L_planet[3] - Pi_planet[3] + 360.0, 360.0));
 double eE = e_planet[3];
 double vE = ME + (2 * eE - 0.25 * pow(eE, 3)) * sin(ME)
                 + (1.25 * pow(eE, 2)) * sin(2 * ME)
                 + ((13.0 / 12.0) * pow(eE, 3)) * sin(3 * ME);
 double rE = a_planet[3] * (1 - eE * eE) / (1 + eE * cos(vE));

 double iE = radians(i_planet[3]);
 double oE = radians(Omega_planet[3]);
 double wE = radians(Pi_planet[3] - Omega_planet[3]);
 ...
 // --- Convert dwarf planet coords from heliocentric to geocentric ---
 double xg = x - xE;
 double yg = y - yE;
 double zg = z - zE;

 // Ecliptic to equatorial conversion (same as before)
 ...
 // Horizontal coordinates, azimuth, altitude, servo control ...
...
// Compute horizontal coordinates and servo movements
double HA = fmod(LST - alpha + TWO_PI, TWO_PI);
...
az = degrees(az);
az = fmod(az + 180.0, 360.0);  // Correct for coordinate system flip
double alt = -degrees(asin(zhor));  // Altitude
...
return;
 ...
 // Classical planets:
 M = radians(fmod(L_planet[planet]-Pi_planet[planet]+360.0,360.0));
 ecc = e_planet[planet];
 double v = M + (2*ecc -0.25*pow(ecc,3))*sin(M)
              + (1.25*pow(ecc,2))*sin(2*M)
              + ((13.0/12.0)*pow(ecc,3))*sin(3*M);
 r = a_planet[planet]*(1 - ecc*ecc)/(1 + ecc*cos(v));
 i_rad = radians(i_planet[planet]);
 o_rad = radians(Omega_planet[planet]);
 w_rad = radians(Pi_planet[planet]-Omega_planet[planet]);
 double v_plus_w = v + w_rad;
...
double alpha = atan2(Yq, Xq);  // RA radians
if (alpha < 0) alpha += TWO_PI;
double delta = atan2(Zq, sqrt(Xq * Xq + Yq * Yq));  // Dec radians
double distance = sqrt(Xq * Xq + Yq * Yq + Zq * Zq);

// Use passed LST (already radians) directly for Hour Angle
double HA = fmod(LST - alpha + TWO_PI, TWO_PI);
